...
import { startScheduler, runExpiryPass, runCredentialSync, runOnlineCheck, syncCredentialGroup, replayPendingOps, archiveEvents, sweepFaceVault, syncUsersTable, closeCredentialGaps, syncCardGrants, refreshSnapshots } from './scheduler.js';
...
app.get('/api/cron/online-check', async (req, res) => {
  ...
  try {
    ...
    try { await refreshSnapshots(10); } catch { /* next run */ }
    try { await syncUsersTable(); } catch { /* next run */ }
    try { await syncCardGrants(); } catch { /* next run */ } // grants mirror card placements
  ...
  try {
    ...
    try {
      ...
      if (!(Number(lastB[0]?.value) > Date.now() - 300000)) {
        ...
        if (!cloudPassive) await refreshSnapshots(4).catch(() => {}); // oldest machine snapshots first
        await syncUsersTable().catch(() => {}); // snapshot-based — safe anywhere
        await syncCardGrants().catch(() => {}); // grants mirror card placements — DB-only
